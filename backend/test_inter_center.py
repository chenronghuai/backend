...
class TestInterCenter(unittest.TestCase, metaclass=TestMeta):
    ...
    def test_match_car_type(self, index):
        ...
        WebDriverWait(self.ic.driver, 5).until(EC.visibility_of_element_located((By.CSS_SELECTOR, '#orderList'))).click()
        self.ic.driver.execute_script("$('tbody.immediate-order').html()")
        self.ic.driver.find_element_by_css_selector('div.nav-right.td-opera > a[title="即时"]').click()
        WebDriverWait(self.ic.driver, 5).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '#order-nav-query'))).click()
        WebDriverWait(self.ic.driver, 5).until(
            EC.visibility_of_all_elements_located((By.CSS_SELECTOR, 'tbody.immediate-order>tr')))
        utils.select_operation_by_attr(self.ic.driver, '#orderImmediately>table', '.immediate-order>tr', 'order-id', character_orders[index-1].order_id, '指派')
        ...
        we_filter_drivers = WebDriverWait(self.ic.driver, 10).until(
        ...
        self.ic.driver.find_element_by_css_selector('#todoExitBtn').click()
        self.ic.driver.switch_to.parent_frame()
        sleep(2)
...
#    @unpack
    def test_appoint(self, order_type):
        ...
        if '指派操作成功!' in getattr(self.ic, 'appoint_result_text', ''):
            ...
            appointed_orders = WebDriverWait(self.ic.driver, 10).until(
            ...
            try:
                ...
                WebDriverWait(self.ic.driver, 10).until(EC.presence_of_all_elements_located(
...
#    @unittest.skip("直接跳过")
    def test_add_order(self, order_type):
        ...
        if '补单操作成功!' in msg_text:
            self.ic.driver.execute_script('$("div#intercityDriver>table>tbody#tdy_driver_queue").html("")')
            sleep(1)
            self.ic.driver.execute_script('$("#order-car-query").click()')
            WebDriverWait(self.ic.driver, 5).until(
    ...
    def test_reback_order(self, _order_type):
        ...
        :return:
        """
        utils.make_sure_driver(self.ic.driver, '监控管理', '城际调度中心', '城际调度中心', 'orderCenterNew.do')  # 不加会机率性导致下句超时，不解？
        WebDriverWait(self.ic.driver, 5).until(EC.visibility_of_element_located((By.CSS_SELECTOR, '#orderList'))).click()
        WebDriverWait(self.ic.driver, 5).until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div.nav-right.td-opera > a[title="已派"]'))).click()
        self.ic.driver.execute_script("$('#tdy_driver_queue').html('')")
        WebDriverWait(self.ic.driver, 5).until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#order-nav-query'))).click()
#        we_appointed_orders = WebDriverWait(self.ic.driver, 10).until(EC.visibility_of_all_elements_located((By.CSS_SELECTOR, 'tbody.dispatched-order>tr')))  # 正式环境暂无class属性
        we_appointed_orders = WebDriverWait(self.ic.driver, 10).until(EC.visibility_of_all_elements_located((By.CSS_SELECTOR, 'tbody>tr[driver_id]')))
...
#                WebDriverWait(self.ic.driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'tbody>tr[driver_id]')))
                sleep(2)
                order_id = we_appointed_orders[i].get_attribute('order-id')
            ...
            if order.order_type == _order_type:
                utils.select_operation_by_attr(self.ic.driver, '#orderImmediately>table', '#orderImmediately>table>tbody>tr[driver_id]', 'order-id', order_id, '改派')
                msg_text = utils.reback_order(self.ic.driver, '司机设备故障')
                if '已成功重新指派订单!' in msg_text:
                    order.order_status = OrderStatus.REWAITING
                    self.ic.driver.execute_script("$('tbody#tdy_driver_queue').html('')")
                    try:
                        we_new_appointed_orders = WebDriverWait(self.ic.driver, 10).until(
                            EC.visibility_of_all_elements_located((By.CSS_SELECTOR, 'tbody>tr[driver_id]')))
...
#    @unittest.skip("直接跳过")
    def test_depart(self, index):
        ...
        :return:
        """
        utils.make_sure_driver(self.ic.driver, '监控管理', '城际调度中心', '城际调度中心', 'orderCenterNew.do')  # 不加这句，以下有可能超时，疑惑？
        ...
        WebDriverWait(self.ic.driver, 15).until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#driverList'))).click()
        self.ic.driver.execute_script('$("#intercityDriver>table>tbody#tdy_driver_queue").html("")')
        WebDriverWait(self.ic.driver, 15).until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'div.bbx-orderlist-nav>.nav-right.td-opera>a[title="专车排班"]'))).click()
        WebDriverWait(self.ic.driver, 5).until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#order-car-query'))).click()
        WebDriverWait(self.ic.driver, 10).until(EC.visibility_of_all_elements_located((By.CSS_SELECTOR, '#intercityDriver>table>tbody#tdy_driver_queue>tr[page_type="driver_queue"]')))
        utils.select_operation_by_attr(self.ic.driver, '#intercityDriver>table', '#intercityDriver>table>tbody>tr[page_type="driver_queue"]', 'driver-id', depart_drivers[index-1].driver_id, '发车')
        ...
        msg_text = utils.wait_for_laymsg(self.ic.driver)
        if '发车成功' in msg_text:
            sleep(1)
            ...
            try:
                ...
                WebDriverWait(self.ic.driver, 5).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, departed_drivers)))
                id_list = [x.get_attribute('driver-id') for x in departed_drivers]
    ...
    def test_operate_menu(self, category):
        ...
        :return:
        """
        result_operate_text = self.ic.get_operate_text(category)
        if category != '已发车':
