...
class FuncDriverReport:
    ...
    def driver_report_by_phone(self, phone, ori_val, des_val):
        ...
        contact_phone = self.driver.find_element_by_css_selector('tbody>tr>td:nth-child(4)').text
        register_phone = self.driver.find_element_by_css_selector('tbody>tr>td:nth-child(5)').text
    ...
    def driver_report_by_carnum(self, carnum, phone, ori_val, des_val):
        ...
        max_user = int(re.search(r'(\d+)\D+(\d+)\D+', license_text).group(1))
        max_package = int(re.search(r'(\d+)\D+(\d+)\D+', license_text).group(2))
        contact_phone = self.driver.find_element_by_css_selector(css_record+'>td:nth-child(4)').text
        register_phone = self.driver.find_element_by_css_selector(css_record + '>td:nth-child(5)').text
    ...
    def driver_cancel_report(self, phone):
        ...
        driver_id = self.driver.find_element_by_css_selector('tbody>tr').get_attribute('data-uid')
        self.driver.execute_script('$("table#data_table>tbody>tr").html("")')  # added by 2021-7-5
        ...
        result_text = utils.wait_for_laymsg(self.driver)  # modified by 2021-7-15
        if '已成功取消' in result_text:
            cancel_driver = globalvar.get_driver(driver_id)
            globalvar.del_driver(cancel_driver)
            return WebDriverWait(self.driver, 5).until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'tbody>tr>td:nth-child(10)'))).text  
        else:
